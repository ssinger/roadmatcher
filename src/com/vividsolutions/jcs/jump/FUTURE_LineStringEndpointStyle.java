...
public abstract class FUTURE_LineStringEndpointStyle extends LineStringStyle implements ChoosableStyle {
    ...
    public FUTURE_LineStringEndpointStyle(String name, Icon icon, boolean start) {
        super(name, icon);
        this.name = name;
        this.icon = icon;                
        this.start = start;
    ...
    private void paint(Coordinate terminal, Coordinate next, Viewport viewport,
        Graphics2D graphics) throws Exception {
        if (!viewport.getEnvelopeInModelCoordinates().contains(terminal)) {
            //Avoid "sun.dc.pr.PRException: endPath: bad path" exceptions [Jon Aquino 11/18/2003]
            return;
        }
